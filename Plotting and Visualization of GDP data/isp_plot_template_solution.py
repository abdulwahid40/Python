# -*- coding: utf-8 -*-
"""
Created on Wed Aug 29 01:35:30 2018
    ...
    for key, value in gdpdata.items():
        if key.isdigit():
            if (int(key) >= gdpinfo["min_year"]) & (int(key) <= gdpinfo["max_year"]):
                try:
                    year_gdp.append((int(key), float(gdpdata[key])))
                except ValueError:
                    continue
                               
    year_gdp.sort(key = lambda pair: pair[0])     
...
def build_plot_dict(gdpinfo, country_list):
    ...
    for country in country_list:
        if country in gdpdata.keys():
            gdp = dict(gdpdata[country])
            country_plot[country] = build_plot_values(gdpinfo, gdp)
        else:
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    gdpdata = build_plot_dict(gdpinfo, country_list)
    
    plot = pygal.XY()
    plot.title = "GDPs of countries by years"
    
    for country in country_list:
        gdp_data = gdpdata[country]
        plot.add(country, gdp_data)
