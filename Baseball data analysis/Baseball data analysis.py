# -*- coding: utf-8 -*-
"""
Created on Wed Aug 22 14:44:25 2018
...
def filter_by_year(statistics, year, yearid):
    ...
    for row in statistics:
        if row[yearid] == str(year):
            filter_year_list.append(row)
            
    return filter_year_list
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        players_list.append(row[info["playerid"]])
        compound_stats_list.append(formula(info, row))    
        
    tuple_list = zip(players_list, compound_stats_list)
    top_players_list = sorted(tuple_list, key = lambda pair: pair[1],
                              reverse = True)
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    master_file = read_csv_as_nested_dict(info["masterfile"], 
                                          info["playerid"], 
                                          info["separator"], 
                                          info["quote"])
    
    for tuple_pair in top_ids_and_stats:
        
        """
        This is another way to find firstname and lastname
        temp = master_file[row[0]]
        firstName = temp[info["firstname"]]
        
        temp = master_file[row[0]]
        lastName = temp[info["lastname"]]
        """ 
        
        first_name = master_file[tuple_pair[0]][info["firstname"]]
        last_name = master_file[tuple_pair[0]][info["lastname"]]        
        stat_player_name = "{:.3f} --- {} {}".format(tuple_pair[1], first_name, 
                            last_name)
        stats_player_list.append(stat_player_name)
    
    return stats_player_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    batting_statistics = read_csv_as_list_dict(info["battingfile"], 
                                               info["separator"],
                                               info["quote"])
    filter_by_year_list = filter_by_year(batting_statistics, year, info["yearid"])
    top_players_stats = top_player_ids(info, filter_by_year_list, formula, numplayers)
    lookup_players_list = lookup_player_names(info, top_players_stats)
    
    return lookup_players_list
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        if row[playerid] not in aggregated_results_dict:
            aggregated_results_dict[row[playerid]] = {}
            aggregated_results_dict[row[playerid]][playerid] = row[playerid]
             
            for item in fields:
                aggregated_results_dict[row[playerid]][item] = float(row[item])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    batting_stats = read_csv_as_list_dict(info["battingfile"], 
                                          info["separator"], 
                                          info["quote"])
    
    aggregated_results = aggregate_by_player_id(batting_stats, 
                                                info["playerid"],
                                                info["battingfields"])
    
    for key in aggregated_results:
        aggregated_list.append(aggregated_results[key])
        
    top_players_stats_by_career = top_player_ids(info, aggregated_list, formula,
                                                 numplayers)
    
    lookup_players_list = lookup_player_names(info, top_players_stats_by_career)
